...
function initializeColumnFilter() {
    ...
    const headers = [
        ...
        'Motifs de rejet URM', 'Parcelles corrigées', 'Geomaticien', 'Parcelles individuelles jointes',
...
function exportData(format = 'csv') {
    ...
    if (format === 'csv') {
        exportToExcel(data); // Now exporting to Excel instead of CSV
    }
}
...
function exportToExcel(data) {
    if (!data.length) {
        showNotification('Pas de données à exporter', 'error');
        return;
    }
    ...
    XLSX.writeFile(wb, `communes_data_${new Date().toISOString().split('T')[0]}.xlsx`);
}
